...
from nodes.sql_query_node import sql_query_node
...
def test_sql_query_node_missing_secret():
    ...
    result = sql_query_node(log, secrets, req)
    assert isinstance(result, QueryResult)
    assert len(result.columns) == 0
    assert len(result.rows) == 0


def test_sql_query_node_rejects_non_select():
    ...
    result = sql_query_node(log, _FakeSecrets(), req)
    assert isinstance(result, QueryResult)
    assert len(result.columns) == 0
